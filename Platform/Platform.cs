...
public partial class Platform : Node2D
{
    ...
    {
        var blockInstance = (Block.Block)_blockScene.Instantiate();
                            
        AddChild(blockInstance);
        blockInstance.Connect(nameof(Block.Block.BlockRemoved), new Callable(this, nameof(_on_BlockRemoved)));
        blockInstance.Position = new Vector2(blockInstance.Position.X + (index * Block.Block.Width), blockInstance.Position.Y);
            
        if (index == 0)
        {
            blockInstance.SetBlockType(Block.Block.BlockTypes.Left);
        } else if (index == BlockLength - 1)
        {
            blockInstance.SetBlockType(Block.Block.BlockTypes.Right);
        }
        else
        {
            blockInstance.SetBlockType(Block.Block.BlockTypes.Middle);
        }

        GD.Randomize();
        if (CanSpawnCoffee && GD.RandRange(0, 100) < PercentChanceCoffee)
        {
            var coffeeInstance = (Coffee.Coffee) _coffeeScene.Instantiate();
